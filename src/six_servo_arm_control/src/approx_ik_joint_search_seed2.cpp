...
int main(int argc, char** argv)
{
  ...
  // Default direction is forward-down: [1, 0, -1], normalized later.
  double dir_x = 1.0;
  double dir_y = 0.0;
  double dir_z = -1.0;

  // Default side direction keeps the two fingers roughly left-right instead of up-down.
  double side_x = 0.0;
  double side_y = 1.0;
  double side_z = 0.0;

  // These defaults should be tuned to your real pre-grasp posture.
  double seed_j0 = 0.0;
  double seed_j1 = 0.9;
  double seed_j2 = 1.57;
  double seed_j3 = -1.57;
  ...
  // Reject candidates whose gripper side axis is too far from the desired side direction.
  ...
  node->get_parameter_or("seed_j0", seed_j0, seed_j0);
  node->get_parameter_or("seed_j1", seed_j1, seed_j1);
  node->get_parameter_or("seed_j2", seed_j2, seed_j2);
  node->get_parameter_or("seed_j3", seed_j3, seed_j3);
  node->get_parameter_or("seed_j4", seed_j4, seed_j4);
  ...
  std::vector<double> seed_q = {seed_j0, seed_j1, seed_j2, seed_j3, seed_j4};
  ...
  for (int i = 0; i < samples; ++i)
  {
    ...
    // The side axis may be flipped left/right and still be acceptable for grasping.
  ...
  RCLCPP_INFO(node->get_logger(), "Seed joints:       [%.4f, %.4f, %.4f, %.4f, %.4f]",
              seed_q[0], seed_q[1], seed_q[2], seed_q[3], seed_q[4]);
