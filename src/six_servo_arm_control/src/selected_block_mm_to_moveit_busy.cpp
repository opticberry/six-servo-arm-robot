...
public:
  ...
  : Node("selected_block_mm_busy_moveit_node")
  {
    ...
    declare_parameter("single_shot_mode", true);
    ...
    single_shot_mode_ = get_parameter("single_shot_mode").as_bool();
    ...
    RCLCPP_INFO(get_logger(), "Busy position-IK node ready.");
    RCLCPP_INFO(
      get_logger(),
      "sample_window_size=%d, spread_threshold=%.4f m, auto_execute=%s, single_shot=%s",
      sample_window_size_,
      sample_spread_threshold_m_,
      auto_execute_ ? "true" : "false",
      single_shot_mode_ ? "true" : "false");
    RCLCPP_INFO(
      get_logger(),
      "pregrasp q=[%.3f, %.3f, %.3f, %.3f, %.3f]",
  ...
  void initMoveIt()
  {
    ...
    RCLCPP_INFO(get_logger(), "MoveIt initialized for group '%s'.", move_group_name_.c_str());
  }
...
private:
  ...
  void workerLoop()
  {
    while (true)
    {
      ...
      if (auto_execute_)
      {
        success = runPregraspAndTargetFlow(target);
      }
      else
      {
        RCLCPP_INFO(
  ...
  bool runPregraspAndTargetFlow(const Target& target)
  {
    RCLCPP_INFO(
      get_logger(),
      "Step 1/3: move to pregrasp, then solve IK for target xyz=(%.3f, %.3f, %.3f)",
      target.x_m,
      target.y_m,
      target.z_m);

    if (!moveToJointTarget(pregrasp_q_, "pregrasp"))
    {
      return false;
    }
    ...
    RCLCPP_INFO(
      get_logger(),
      "Step 2/3: IK succeeded. error=%.4f m, q=[%.4f, %.4f, %.4f, %.4f, %.4f]",
      final_pos_error,
      ik_q[0], ik_q[1], ik_q[2], ik_q[3], ik_q[4]);

    if (!moveToJointTarget(ik_q, "target"))
    {
      return false;
    }

    moveit::core::RobotState executed_state(robot_model_);
    executed_state.setToDefaultValues();
    executed_state.setJointGroupPositions(jmg_, ik_q);
    executed_state.update();

    const Eigen::Vector3d executed_p = executed_state.getGlobalLinkTransform("tool0").translation();
    const double ex = executed_p.x() - target.x_m;
    const double ey = executed_p.y() - target.y_m;
    const double ez = executed_p.z() - target.z_m;
    const double exec_pos_err = std::sqrt(ex * ex + ey * ey + ez * ez);

    RCLCPP_INFO(
      get_logger(),
      "Step 3/3: target reached in plan model. xyz=[%.4f, %.4f, %.4f], err=%.4f m",
      executed_p.x(),
      executed_p.y(),
      executed_p.z(),
      exec_pos_err);
  ...
  bool moveToJointTarget(const std::vector<double>& q, const std::string& label)
  {
    if (!move_group_)
    {
    ...
    {
      RCLCPP_ERROR(get_logger(), "Failed to set %s joint target.", label.c_str());
      return false;
    }
    ...
    {
      RCLCPP_ERROR(get_logger(), "Planning to %s failed.", label.c_str());
      return false;
    }

    RCLCPP_INFO(get_logger(), "Planning to %s succeeded. Executing...", label.c_str());
    if (move_group_->execute(plan) != moveit::core::MoveItErrorCode::SUCCESS)
    {
      RCLCPP_ERROR(get_logger(), "Execution to %s failed.", label.c_str());
      return false;
    }

    const int time_ms = (label == "pregrasp") ? pregrasp_time_ms_ : target_time_ms_;
