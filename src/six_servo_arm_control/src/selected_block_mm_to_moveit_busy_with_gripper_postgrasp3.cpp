...
public:
  ...
  : Node("selected_block_base_busy_moveit_node")
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Local servo: enabled=%s topic=%s gain=%.3f sign=(%.1f, %.1f) max_step=%.1fmm stop=%.1fmm max_iters=%d",
...
private:
  ...
  bool runLocalServoLoop(Target& current_target, std::vector<double>& current_q)
  {
    for (int iter = 0; iter < local_servo_max_iters_; ++iter)
    {
      ...
      double step_x_m = clampAbs(rule_servo_gain_ * error_x_m, local_servo_max_step_m_);
      double step_y_m = clampAbs(rule_servo_gain_ * error_y_m, local_servo_max_step_m_);
      ...
      RCLCPP_INFO(
        get_logger(),
        "Local servo iter=%d tool0_now=(%.3f, %.3f, %.3f) error_xy=(%.1f, %.1f)mm step_xy=(%.1f, %.1f)mm next=(%.3f, %.3f, %.3f)",
        iter + 1,
        tool0_now.x_m,
        tool0_now.y_m,
        tool0_now.z_m,
      ...
      if (!moveArmToJointTarget(next_q, "local_servo", local_servo_time_ms_))
      {
        return false;
      }
