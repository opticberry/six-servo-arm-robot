...
public:
  ...
  : Node("selected_block_base_busy_moveit_node")
  {
    ...
    declare_parameter("servo_time_scale", 1.3);
    declare_parameter("servo_time_offset_ms", 0);
    ...
    declare_parameter("release_after_postgrasp", false);
    ...
    servo_time_scale_ = get_parameter("servo_time_scale").as_double();
    servo_time_offset_ms_ = get_parameter("servo_time_offset_ms").as_int();
    ...
    release_after_postgrasp_ = get_parameter("release_after_postgrasp").as_bool();
    ...
    RCLCPP_INFO(
      get_logger(),
      "MoveIt plan execution: direct /arm_joint_cmd topic=%s min_point_dt=%dms default_gripper=%.3f servo_time_scale=%.2f servo_time_offset=%dms",
      direct_joint_topic_.c_str(),
      moveit_plan_min_point_dt_ms_,
      moveit_plan_default_gripper_position_,
      servo_time_scale_,
      servo_time_offset_ms_);
    RCLCPP_INFO(
      get_logger(),
...
private:
  ...
  bool runPregraspAndTargetFlow(const Target& target)
  {
    ...
    if (!moveArmToJointTarget(pregrasp_q_, "pregrasp", pregrasp_time_ms_, gripper_open_))
    {
      return false;
    }
    ...
    if (!moveArmToJointTarget(ik_q, "target", target_time_ms_, gripper_open_))
    {
      return false;
    }
    ...
    if (!moveArmToJointTarget(postgrasp_q_, "postgrasp", postgrasp_time_ms_, gripper_close_))
    {
      return false;
    }

    if (release_after_postgrasp_)
    {
      publishDirectJointPose(postgrasp_q_, gripper_open_, "gripper_open_direct", gripper_settle_ms_);
      std::this_thread::sleep_for(std::chrono::milliseconds(gripper_settle_ms_));
    }
    else
    {
      RCLCPP_INFO(get_logger(), "release_after_postgrasp=false, keep gripper closed after postgrasp.");
    }
  ...
  {
    // In direct /arm_joint_cmd execution, MoveIt's current state can lag behind
    // the hardware command. Use the tracked joint vector as the local-servo state.
    ...
    const std::vector<double>& target_q)
  {
    RCLCPP_INFO(
      ...
      target_q[3],
      target_q[4]);
    return moveArmToJointTarget(target_q, "local_servo", local_servo_time_ms_, gripper_open_);
  }
    ...
    double gripper_position)
  {
    ...
    if (!executeMoveItPlanViaArmCmd(plan, label, gripper_position))
    {
      return false;
    }
    ...
    const std::string& label)
  {
    ...
    if (!getLastCommandedPosition(position) || !jmg_)
    {
      return false;
    }
    ...
    double gripper_position)
  {
    ...
    {
      RCLCPP_ERROR(get_logger(), "Arm joint command publisher is not initialized.");
      return false;
    }
    ...
    RCLCPP_INFO(
      get_logger(),
      "Executing MoveIt plan %s via /arm_joint_cmd: points=%zu gripper=%.3f",
      label.c_str(),
      ...
      ++point_index)
    {
      ...
      const int servo_time_ms = std::max(
        moveit_plan_min_point_dt_ms_,
        static_cast<int>(std::llround(segment_dt_ms * servo_time_scale_)) +
        servo_time_offset_ms_);
      ...
      publishArmJointCommand(cmd_position, servo_time_ms);
...
private:
  ...
  double servo_time_scale_{1.3};
  int servo_time_offset_ms_{0};
  ...
  bool release_after_postgrasp_{false};
