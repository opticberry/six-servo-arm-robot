...
private:
    ...
    const std::vector<double>& target_q)
  {
    ...
    if (local_servo_execute_mode_ == "direct_joint")
    {
      return executeDirectJointRamp(current_q, target_q);
    }
    ...
    const std::vector<double>& target_q)
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Direct local_servo ramp: interpolation=%s steps=%d period=%dms max_delta=%.4frad target=[%.4f, %.4f, %.4f, %.4f, %.4f]",
      direct_joint_interpolation_.c_str(),
      steps,
      direct_joint_period_ms_,
      max_abs_delta,
    ...
    for (int step = 1; step <= steps && rclcpp::ok(); ++step)
    {
      ...
      cmd_position[5] = static_cast<float>(gripper_open_);
    ...
    printStm32RadCommands("local_servo_direct_joint", target_q, direct_joint_move_time_ms_);
    return true;
  }
