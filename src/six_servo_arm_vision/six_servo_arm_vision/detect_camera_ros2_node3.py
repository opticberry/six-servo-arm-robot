...
def load_eye_to_hand(eye_to_hand_path: str) -> dict | None:
...
def pixel_to_base_point(
    ...
    u, v = float(center_xy[0]), float(center_xy[1])
    ...
    scale = (float(table_z_m) - float(camera_origin_base[2])) / float(ray_base[2])
    if scale <= 0.0:
        return None

    point_base = camera_origin_base + scale * ray_base
    return np.round(point_base, decimals=6).tolist()
...
class DetectCameraRknnNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("table_z_m", 0.02)
        self.declare_parameter("base_frame", "base_link")
        self.declare_parameter("target_color", "red")
        self.declare_parameter("camera_frame", "camera")
        self.declare_parameter("use_last_selected_when_missing", True)
        self.declare_parameter("last_selected_timeout_s", 0.0)
        self.declare_parameter("publish_cached_selected_base", False)
        self.declare_parameter("enable_gripper_marker_error", True)
        ...
        self.declare_parameter("visual_servo_use_marker_z", False)
        self.declare_parameter("visual_servo_error_bias_x_m", 0.0)
        self.declare_parameter("visual_servo_error_bias_y_m", 0.0)
        self.declare_parameter("visual_servo_error_bias_z_m", 0.0)
        ...
        self.eye_to_hand_path = str(self.get_parameter("eye_to_hand").value)
        self.table_z_m = float(self.get_parameter("table_z_m").value)
        self.base_frame = str(self.get_parameter("base_frame").value)
        self.target_color = str(self.get_parameter("target_color").value)
        self.camera_frame = str(self.get_parameter("camera_frame").value)
        self.use_last_selected_when_missing = bool(
            self.get_parameter("use_last_selected_when_missing").value
        )
        self.last_selected_timeout_s = float(
            self.get_parameter("last_selected_timeout_s").value
        )
        self.publish_cached_selected_base = bool(
            self.get_parameter("publish_cached_selected_base").value
        )
        self.enable_gripper_marker_error = bool(
            self.get_parameter("enable_gripper_marker_error").value
        )
        ...
        self.marker_dictionary = str(self.get_parameter("marker_dictionary").value)
        self.marker_length_m = float(self.get_parameter("marker_length_m").value)
        self.visual_servo_use_marker_z = bool(
            self.get_parameter("visual_servo_use_marker_z").value
        )
        self.visual_servo_error_bias = np.array(
            [
                float(self.get_parameter("visual_servo_error_bias_x_m").value),
                float(self.get_parameter("visual_servo_error_bias_y_m").value),
                float(self.get_parameter("visual_servo_error_bias_z_m").value),
            ],
            dtype=np.float64,
        )
        ...
        self.active_camera_matrix = (
            self.calibration["camera_matrix"].astype(np.float64)
            if self.calibration is not None
            else None
        )
        self.marker_dist_coeffs = (
            self.calibration["dist_coeffs"].astype(np.float64)
            if self.calibration is not None
            else None
        )
        ...
        self.selected_base_pub = self.create_publisher(
            PointStamped, "/selected_block_base", 10
        )
        self.selected_color_pub = self.create_publisher(
            String, "/selected_block_color", 10
        )
        self.gripper_left_pub = self.create_publisher(
            PointStamped, "/gripper_left_marker_base", 10
        )
        self.gripper_right_pub = self.create_publisher(
            PointStamped, "/gripper_right_marker_base", 10
        )
        self.gripper_center_pub = self.create_publisher(
            PointStamped, "/gripper_center_base", 10
        )
        self.visual_servo_error_pub = self.create_publisher(
            PointStamped, "/visual_servo_error_base", 10
        )
        self.selected_debug_pub = self.create_publisher(
            String, "/selected_block_debug", 10
        )
        self.visual_servo_debug_pub = self.create_publisher(
            String, "/visual_servo_debug", 10
        )
        self.all_debug_pub = self.create_publisher(String, "/detected_blocks_debug", 10)

        self.aruco_detector = None
        self.last_selected_base_m: list[float] | None = None
        self.last_selected_detection: dict | None = None
        self.last_selected_update_time = 0.0
        if self.enable_gripper_marker_error:
            if self.eye_to_hand is None or self.active_camera_matrix is None:
                self.get_logger().warning(
                    "Gripper marker error disabled because calibration or eye_to_hand is missing."
                )
                self.enable_gripper_marker_error = False
            else:
                self.aruco_detector = make_aruco_detector(self.marker_dictionary)
        ...
        if self.eye_to_hand is not None:
            self.get_logger().info(
                "Eye-to-hand: %s, table_z_m=%.3f, base_frame=%s"
                % (self.eye_to_hand["path"], self.table_z_m, self.base_frame)
            )
        if self.enable_gripper_marker_error:
            self.get_logger().info(
                "Gripper marker error enabled: left_id=%d, right_id=%d, marker=%.3fm, bias=(%.3f, %.3f, %.3f)m, use_marker_z=%s"
                % (
                    self.left_marker_id,
                    self.right_marker_id,
                    self.marker_length_m,
                    self.visual_servo_error_bias[0],
                    self.visual_servo_error_bias[1],
                    self.visual_servo_error_bias[2],
                    self.visual_servo_use_marker_z,
                )
            )
        if self.show:
    ...
    def _get_cached_selected_base(self) -> tuple[list[float] | None, dict | None, str | None]:
        if not self.use_last_selected_when_missing:
            return None, None, None
        if self.last_selected_base_m is None:
            return None, None, None

        if self.last_selected_timeout_s > 0.0:
            age_s = time.monotonic() - self.last_selected_update_time
            if age_s > self.last_selected_timeout_s:
                return None, None, None

        return self.last_selected_base_m, self.last_selected_detection, "cached"

    def _publish_base_point(
        self,
        publisher,
        point_xyz: list[float] | np.ndarray,
        frame_id: str | None = None,
    ) -> None:
        msg = PointStamped()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.header.frame_id = frame_id or self.base_frame
        msg.point.x = float(point_xyz[0])
        msg.point.y = float(point_xyz[1])
        msg.point.z = float(point_xyz[2])
        publisher.publish(msg)

    def _estimate_gripper_marker_points(
        self,
        frame: np.ndarray,
    ) -> tuple[dict[int, np.ndarray], object]:
        if not self.enable_gripper_marker_error or self.aruco_detector is None:
            return {}, None

        corners_list, ids, _ = detect_markers(frame, self.aruco_detector)
        marker_base: dict[int, np.ndarray] = {}
        if ids is None:
            return marker_base, None
        ...
        for marker_id, corners in zip(ids.flatten().tolist(), corners_list):
            ...
            marker_base[int(marker_id)] = transform_point(
                self.eye_to_hand["T_base_camera"],
                center_camera,
            )

        return marker_base, ids
    ...
    def _timer_callback(self) -> None:
        ...
        if self.calibration is not None:
            ...
            if self.undistort_maps is None:
                ...
                self.active_camera_matrix = self.undistort_maps[2].astype(np.float64)
                self.marker_dist_coeffs = np.zeros((5, 1), dtype=np.float64)
            frame = cv2.remap(
        ...
        if self.eye_to_hand is not None and self.active_camera_matrix is not None:
            for det in detections:
                det["center_base_m"] = pixel_to_base_point(
                    det["center_xy"],
                    self.active_camera_matrix,
                    self.eye_to_hand["T_base_camera"],
                    self.table_z_m,
        ...
        marker_base, marker_ids = self._estimate_gripper_marker_points(frame)
        left_marker_base = marker_base.get(self.left_marker_id)
        right_marker_base = marker_base.get(self.right_marker_id)
        gripper_center_base = None
        if left_marker_base is not None:
            self._publish_base_point(self.gripper_left_pub, left_marker_base)
        if right_marker_base is not None:
            self._publish_base_point(self.gripper_right_pub, right_marker_base)
        if left_marker_base is not None and right_marker_base is not None:
            gripper_center_base = (left_marker_base + right_marker_base) * 0.5
            self._publish_base_point(self.gripper_center_pub, gripper_center_base)
            ...
            debug_msg.data = json.dumps(
                {
                    ...
                    "gripper_left_base_m": None
                    if left_marker_base is None
                    else np.round(left_marker_base, 6).tolist(),
                    "gripper_right_base_m": None
                    if right_marker_base is None
                    else np.round(right_marker_base, 6).tolist(),
                    "gripper_center_base_m": None
                    if gripper_center_base is None
                    else np.round(gripper_center_base, 6).tolist(),
                },
                ensure_ascii=True,
            )
        ...
        if (
            ...
            and self.eye_to_hand is not None
            and self.active_camera_matrix is not None
        ):
            selected_base = selected.get("center_base_m")
            if selected_base is not None:
                self.last_selected_base_m = list(selected_base)
                self.last_selected_detection = dict(selected)
                self.last_selected_update_time = time.monotonic()
                selected_source = "live"

        if selected_base is None:
            cached_base, cached_detection, cached_source = self._get_cached_selected_base()
            if cached_base is not None:
                selected_base = list(cached_base)
                if selected is None and cached_detection is not None:
                    selected = dict(cached_detection)
                selected_source = cached_source

        visual_servo_error = None
        raw_visual_servo_error = None
        if selected_base is not None and gripper_center_base is not None:
            raw_visual_servo_error = (
                np.array(selected_base, dtype=np.float64) - gripper_center_base
            )
            visual_servo_error = raw_visual_servo_error.copy()
            if not self.visual_servo_use_marker_z:
                visual_servo_error[2] = 0.0
            visual_servo_error += self.visual_servo_error_bias
            self._publish_base_point(self.visual_servo_error_pub, visual_servo_error)

        should_publish_selected_base = (
            selected_base is not None
            and (selected_source == "live" or self.publish_cached_selected_base)
        )
        if should_publish_selected_base:
            base_msg = PointStamped()
            base_msg.header.stamp = self.get_clock().now().to_msg()
            base_msg.header.frame_id = self.base_frame
            base_msg.point.x = float(selected_base[0])
            base_msg.point.y = float(selected_base[1])
            base_msg.point.z = float(selected_base[2])
            self.selected_base_pub.publish(base_msg)

        if selected is not None:
            if selected["center_xy_mm"] is not None:
            ...
            debug_msg.data = json.dumps(
                {
                    ...
                    "selected": selected,
                    "selected_base_m": selected_base,
                    "selected_source": selected_source,
                    "gripper_center_base_m": None
                    if gripper_center_base is None
                    else np.round(gripper_center_base, 6).tolist(),
                    "raw_visual_servo_error_m": None
                    if raw_visual_servo_error is None
                    else np.round(raw_visual_servo_error, 6).tolist(),
                    "visual_servo_error_m": None
                    if visual_servo_error is None
                    else np.round(visual_servo_error, 6).tolist(),
                    "visual_servo_error_norm_m": None
                    if visual_servo_error is None
                    else round(float(np.linalg.norm(visual_servo_error)), 6),
            ...
            if visual_servo_error is not None:
                servo_debug = String()
                servo_debug.data = json.dumps(
                    {
                        "selected_block_base_m": np.round(
                            np.array(selected_base, dtype=np.float64), 6
                        ).tolist(),
                        "gripper_center_base_m": np.round(gripper_center_base, 6).tolist(),
                        "raw_error_m": np.round(raw_visual_servo_error, 6).tolist(),
                        "bias_m": np.round(self.visual_servo_error_bias, 6).tolist(),
                        "visual_servo_error_m": np.round(visual_servo_error, 6).tolist(),
                        "visual_servo_error_norm_m": round(
                            float(np.linalg.norm(visual_servo_error)), 6
                        ),
                        "visual_servo_use_marker_z": self.visual_servo_use_marker_z,
                    },
                    ensure_ascii=True,
                )
                self.visual_servo_debug_pub.publish(servo_debug)
            ...
            self.get_logger().info(
                ...
                f"center_base_m={selected_base} "
                f"gripper_center_base_m="
                f"{None if gripper_center_base is None else np.round(gripper_center_base, 4).tolist()} "
                f"visual_servo_error_m="
                f"{None if visual_servo_error is None else np.round(visual_servo_error, 4).tolist()} "
        ...
        if self.show:
            ...
            if marker_ids is not None:
                marker_corners, marker_id_values, _ = detect_markers(annotated, self.aruco_detector)
                if marker_id_values is not None:
                    cv2.aruco.drawDetectedMarkers(annotated, marker_corners, marker_id_values)

            if visual_servo_error is not None:
                error_text = (
                    f"err_xy_mm=({visual_servo_error[0] * 1000.0:.1f}, "
                    f"{visual_servo_error[1] * 1000.0:.1f})"
                )
                if self.visual_servo_use_marker_z:
                    error_text += f" z={visual_servo_error[2] * 1000.0:.1f}"
                cv2.putText(
                    annotated,
                    error_text,
                    (20, 30),
